...
- **The rising floor (a survival game: the meta's `survival` is `"easy"`,
  `"normal"` or `"hard"`; cooperative only).** Nobody attacks anybody: the
  ...
  end, in whole nanoseconds — easy 2.5 s → 1.0 s, normal 3.0 s → 1.5 s,
  hard 5.0 s → 1.0 s: `start − (start − end) × (level − 1) / 14`. When it
  ...
  written), where `rows` is what the (K+1)-th raise brings: 1 on easy, 4 on
  hard, and on normal `1 + PCG(seed ^ 0x5375727620726f77, K+1).IntN(4)` (Go's
