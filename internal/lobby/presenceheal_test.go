...
func TestPresenceHealsWhenGameDeleted(t *testing.T) {
	...
	gameID, err := lb.CreateGame(ctx, config.ModeCompetitive, 2, 0, 0, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPresenceUntouchedWhenOtherGameDies(t *testing.T) {
	...
	otherID, err := lb.CreateGame(ctx, config.ModeCompetitive, 2, 0, 0, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
	gameID, err := lb.CreateGame(ctx, config.ModeCompetitive, 2, 0, 0, config.GameRules{Ghost: true}, false)
