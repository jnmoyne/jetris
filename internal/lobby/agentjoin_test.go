...
func TestAgentJoinPolicy(t *testing.T) {
	...
	noAgents, err := human.CreateGame(ctx, config.ModeCompetitive, 2, 0, 0, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	oneAgent, err := human.CreateGame(ctx, config.ModeCompetitive, 3, 0, 1, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentsConcurrentJoinsRespectCap(t *testing.T) {
	...
	gameID, err := human.CreateGame(ctx, config.ModeCompetitive, 4, 0, 1, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
