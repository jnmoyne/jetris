...
**Spawn blocked (shared boards):** the same distinction gravity makes applies at spawn time. If the spawn cells are held by **locked cells**, the player tops out. If they are covered **only by another player's active (falling) piece** — a transient obstacle that will itself fall away — the spawn does **not** top out: it is deferred and retried as soon as the shared board changes (every incoming cell message may be the blocker moving away — at agent speeds a piece crosses the spawn cells in milliseconds), with the housekeeping tick as the backstop, until it succeeds, the cells become locked (a genuine top-out), or the blockers turn out to be nobody's live piece: the same cells at the same stream sequences — not rewritten since — blocking for `SpawnBlockedVacateAfter` (3 s; a falling piece is rewritten every row it falls, one resting on the stack locks a lock delay after its last shift, every shift a rewrite) are a piece left behind, and the blocked engine vacates that piece whole, at those sequences, and spawns (`deferSpawnLocked`; a stale piece once held a spawn box for the rest of a game). The moves the player pressed behind their own hard drop are held for the next piece for the round trip its lock and spawn take (`awaitSpawn`); a spawn deferred past `SpawnHoldRelease` (1 s) is not that round trip, so the hold ends there — the held moves are let go with a flash on the spawn cells, and later presses are no-ops against the empty board rather than a queue that grows by the tap. Detection mirrors movement: `CanPlaceCoop` fails but `CanPlace` (which ignores active cells) succeeds. Without this rule, a teammate's piece merely crossing the spawn area would spuriously eliminate the player (and in cooperative end the game for everyone). The engine also runs a piece-less **watchdog** on the same gravity tick: an alive player with no piece and no deferred spawn for two consecutive ticks gets a forced (re)spawn, healing a spawn whose publish was lost on a board that has since gone silent. Neither the deferral retry nor the watchdog runs before the game starts.
...
5. Each crew member's own share of the score — the cumulative `total_score` their events carry, their own read live — is listed, muted, beside their name in the HUD's PLAYERS list, in roster order (the crew is not ranked; the total stays the SCORE stat), for players and spectators alike, and a replay's board carries the same line under its label (§8)
...
player's beside their name, best first (your own row in the accent colour; a
spectator sees the same ranking in place of a SCORE stat, and a replay's board
carries it), and a peer's `line_clear` folds the
...
Competitive scores by the same modern scoring table (§2 Scoring): each player keeps their own score — their clears, T-spins, Back-to-Back, combos and drop points — multiplied by the level their **own** line total reaches (`1 + lines / 10`, shown as the HUD's LEVEL — the level their gravity falls at too). The score decides nothing: the winner is the last player left who has not topped out, and the score is kept for the leaderboard. Every scoring lock publishes a `line_clear` here too, carrying the player's cumulative `total_score`/`total_lines`: nobody folds those points into a score of their own, but every engine — every player's, every spectator's — keeps every player's totals from them (`Engine.PlayerScores`), so the HUD's PLAYERS list shows every player's score beside their name, best first, the player's own row in the accent colour; a spectator reads each player's level there too (every private board falls at its own, so a competitive spectator's HUD has no SCORE or LEVEL stat of its own) and each board carries its player's score beside the name; the game-over box lists everyone, best first, under `Your score`; and the line goal and the archive read the same totals. The player's `game_over` carries them once more, so their last points count everywhere.
...
- HUD shows `Teams · TEAM A/B/…`, a live per-team scoreboard (one row per team, own team highlighted), and the team level; spectators instead see each team's score **and level** inline (`42 · lvl 3`) with no single SCORE/LEVEL stat; the legend lists each member's own points, muted, beside their name under their team's header, and a spectator's team boards carry the team's score beside the label
...
- **Open games** list in the lobby with Join/Spectate buttons, and the
  **"Allow agents to join"** checkbox and max-agents count (per team in a
  ...
  piece left behind by a player who crashed or dropped off — or a copy of
  one an older client's stale collapse stranded beside it — is vacated by
  the other players' engines once none of its cells has been rewritten for
  10 s (the clock runs per cell, the cells grouped by the anchor they
  claim, so a seat's live piece never keeps a stray of the same seat's
  alive; a live piece never stands that long), or the moment its seat is
  seen empty; and with several
...
The record's `version` says what its numbers mean: 2, the levels are the ones the game shows (1 at the start, §7); 3, every player's `score` is their OWN locks' total in every mode — the `total_score` their events announce (a 2's shared-board rows mixed the board's total, on the archiver's and the topped-out players' rows, with the survivors' own, so the history lists each crew member's share only from 3 on — `ArchiveRecord.OwnScores`; a board scored per seat always carried them); a record with no version was written when levels were 0-based, and every reader raises its levels by one on decode (`ArchiveRecord.Normalize`, which leaves a 2 a 2: what it lacks cannot be recomputed), so the history reads the same for old games and new. Each player result carries the `level` achieved at game end (derived from that engine's line total; sent in `EventGameOver`), the `lines` the player's own pieces cleared (the same event's `total_lines`; absent in records written before the field) and an `agent` flag (from the roster at archive time) marking seats that were played by agents. Cooperative records carry the shared `total_score` and `final_level`; the history list shows them, with each member's share beside their name (best first) from version 3 on and the names alone before, while a board scored per seat — competitive, or the crew's board scored individually — lists each player's score and level, winner(s) first.
...
- **Player legend:** Shows each player's name with their assigned color swatch — and their score: on a board scored per seat every seat's, best first (competitive: with each player's level, the boards falling at their own — the SCORE and LEVEL stats stay off a competitive spectator's HUD; a compact bar shows the ranking instead), on a shared board each member's share of the crew's or the team's total. A competitive spectator's boards carry each player's score beside the name, a teams spectator's the team's
...
- **Replays** (the history's Replay button; README): a recorded game plays back with the same scoreboard, one muted line under each board's label that follows the playhead — a competitive board's score and level (`1200 · lvl 3`), a team's total and level then each member's own points (`1200 · lvl 3 — alice 700 · bob 500`), the crew's total and level then each member's share, or the seats of a board scored per seat ranked best first — read off the recorded `line_clear`/`game_over` totals exactly as a live spectator's engine folded them, and rebuilt from nothing on a scrub back like the boards are. A recording made before the events carried totals shows none. The scrubber's clear markers come from the same events on every board that announced its clears, and off the boards themselves (the cell count dropping a row's worth in one batch) only for a competitive recording from before competitive boards published them
...
In **cooperative mode** the players share the same cell subjects, but with per-cell granularity only writes to the **same cell** actually contend — two players moving in different parts of the board (even on the same row) never conflict, so contention is rare. On CAS failure the engine refetches the latest message for every affected cell in one batched round trip and merges per cell: this player's content is kept except where the stream now holds the other player's mid-flight (active) piece — those cells are skipped entirely, never overwritten or vacated, and if one of them is where this player's piece (or its lock) was going, the whole step is dropped rather than a piece published in part. Nor does the merge ever write another seat's active cell: a copy of their piece carried over from the projection the race was lost on is dropped — their piece is wherever the stream has it now, and a copy beside it once duplicated the piece on the wire, whose orphaned cells then held a spawn box for the rest of the game. The crew's line-clear collapse, which carries the other seats' pieces down with the stack, is not merged at all: lost, it is projected again off a full server snapshot and published whole (`publishCoopTransform`, up to 12 attempts). It then retries the atomic batch with refreshed per-subject CAS expectations (up to 16 attempts, with a short per-player-offset backoff between tries that breaks lockstep with the other player's retry loop).
