...
func TestInviteFlow(t *testing.T) {
	...
	gameID, err := a.CreateGame(ctx, config.GameSpec{Mode: config.ModeCompetitive, PlayerCount: 3, Rules: config.GameRules{Ghost: true}, InviteOnly: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInviteDecline(t *testing.T) {
	...
	gameID, err := a.CreateGame(ctx, config.GameSpec{Mode: config.ModeTeams, PlayerCount: 2, TeamCount: 2, TeamSize: 1, Rules: config.GameRules{Ghost: true}, InviteOnly: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInviteMultiple(t *testing.T) {
	...
	g1, err := a.CreateGame(ctx, config.GameSpec{Mode: config.ModeCompetitive, PlayerCount: 2, Rules: config.GameRules{Ghost: true}, InviteOnly: true})
	if err != nil {
		t.Fatal(err)
	}
	g2, err := b.CreateGame(ctx, config.GameSpec{Mode: config.ModeCooperative, PlayerCount: 2, Rules: config.GameRules{Ghost: true}, InviteOnly: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInviteRetract(t *testing.T) {
	...
	gameID, err := a.CreateGame(ctx, config.GameSpec{Mode: config.ModeCompetitive, PlayerCount: 2, Rules: config.GameRules{Ghost: true}, InviteOnly: true})
	if err != nil {
		t.Fatal(err)
	}
	if err := a.Invite(ctx, b.PlayerID(), gameID, 0); err != nil {
