...
- **LAN mode (embedded NATS server):** Jetris starts a JetStream-enabled `nats-server` inside the game process itself — default account, no auth, JetStream data in a local `jetstream-data` directory — and connects to it. Both the IP and the port are editable (the IP is pre-filled with your machine's auto-detected LAN address, the port with 4222; typing in either selects this option). The screen shows "Your server's URL is `nats://<ip>:<port>`" so you can share the address with other players, who connect to it via their NATS URL field — override the IP when the detected one isn't the address your friends can reach (multiple network interfaces, a VPN, a container). The server always listens on *every* interface, so the IP field only changes which address Jetris advertises and connects through. The server keeps running until you close the window, so quitting to the login screen doesn't kick your friends.

A **Check connection** button really dials the current choice and shows the server it reached plus its **Core NATS ping** — the round trip of an actual message published to an inbox and received back from the server — without joining anything. With LAN mode selected it reads **Check embedded server**: it starts the embedded server and pings it over the address other players will dial. Hitting **Play** connects and logs in. Quitting the lobby returns to this screen, so you can switch servers without restarting.
