...
type GameMeta struct {
    ...
    // Hold: the Guideline hold queue is on — a seat may swap its falling piece
    ...
    // the Guideline way. Presentation only: play, the engine's visible
...
// nothing, a double 1 row, a triple 2, a Jetris 4. handleLockIn passes its
...
// The Guideline hold (GameMeta.Hold). Hold dispatches MoveHold like any
...
const (
    ...
    RotateCCW
    MoveHardDrop
    MoveHold    // the Guideline hold (Engine.Hold)
...
In **competitive mode** each player keeps its own local score counter (`score atomic.Int64`), advanced by the Guideline's points for each lock (`game.Clear.Points` at the player's own level, plus drop points). The score is reported to other clients only at game end via the `EventGameOver` event (and rendered locally via `UpdateScore`); the per-player `score` subject is not used.

In **teams mode** each team's score follows the cooperative scheme: a lock adds the Guideline's points, published as `EventLineClear{Team, Score, LinesCleared, TotalScore, TotalLines}` and folded by every same-team engine — **both** the score delta and the line count (as coop does), so teammates' levels and gravity intervals stay in sync with the team total. Events from the other team are never folded into the own-team `score`; their garbage arrives through the board's garbage register, not an event.
...
// "guideline garbage" — or just "guideline" when the rules are
// config.GuidelineRules, the wizard's preset).
...
type GameListing struct {
    ...
    NoGhost            bool       `json:"no_ghost,omitempty"`             // ghost off, mirrors GameMeta.NoGhost (so the "guideline" tag matches the preset exactly)
