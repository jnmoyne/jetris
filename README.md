...
A finished game has a link of its own as well. The replay screen's **Share** button puts that replay's link on the screen as a QR code, with the link under it and a **Copy link** button: it is the same join page with the game named — `join.html?server=…&name=…&replay=<gameID>` — so whoever opens it (or scans it) types a name and lands straight on the replay screen with that game playing. The desktop build sends the link to the GitHub Pages copy of the game and the server's WebSocket address (the official servers are bookmarked both ways, so a desktop on the `nats://` one still knows it; a LAN party shares its own page); the desktop opens the same game with `jetris --server … --name alice --replay <gameID>`. Beside Share sits **Pin**: a pinned replay is never cleaned up — replays are otherwise kept only while the game is among the 25 most recent or the top 10 of its bucket — until someone presses **Unpin**; the history marks pinned games `PINNED`.
