...
// round trip).
func (a *App) openInvitePicker(mode config.GameMode, count int, rules config.GameRules) {
	gameID := a.createGame(mode, count, 0, rules, true) // invite-only: agent policy is per-invite
	if gameID == "" {
		return
	}
	lb := a.getLobby()
