...
`--survival`, the rising floor of a cooperative game, `survival`: `too_easy`,
`super_easy`, `very_easy`, `easy`, `normal` or `hard` — garbage rows rise from
the bottom of the crew's board on a clock that quickens with the level until the
crew tops out, the time survived the result (no
