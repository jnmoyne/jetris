...
func TestAgentJoinPolicy(t *testing.T) {
	...
	noAgents, err := human.CreateGame(ctx, config.ModeCompetitive, 2, 0, 0, 0, false, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
	...
	oneAgent, err := human.CreateGame(ctx, config.ModeCompetitive, 3, 0, 0, 1, false, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAgentsConcurrentJoinsRespectCap(t *testing.T) {
	...
	gameID, err := human.CreateGame(ctx, config.ModeCompetitive, 4, 0, 0, 1, false, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnjoinGame(t *testing.T) {
	...
	gameID, err := a.CreateGame(ctx, config.ModeCompetitive, 2, 0, 0, 0, false, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
