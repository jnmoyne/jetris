...
func TestInviteFlow(t *testing.T) {
	...
	gameID, err := a.CreateGame(ctx, config.ModeCompetitive, 3, 0, 0, 0, config.GameRules{Ghost: true}, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInviteDecline(t *testing.T) {
	...
	gameID, err := a.CreateGame(ctx, config.ModeTeams, 2, 1, 0, 0, config.GameRules{Ghost: true}, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInviteMultiple(t *testing.T) {
	...
	g1, err := a.CreateGame(ctx, config.ModeCompetitive, 2, 0, 0, 0, config.GameRules{Ghost: true}, true)
	if err != nil {
		t.Fatal(err)
	}
	g2, err := b.CreateGame(ctx, config.ModeCooperative, 2, 0, 0, 0, config.GameRules{Ghost: true}, true)
	if err != nil {
		t.Fatal(err)
	}
