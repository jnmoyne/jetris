...
*Spectating a 2v2 teams game with the modern attack table, a while in: Team A has just completed two lines at once (a double, flashing white) and the garbage line that clear sends under the modern attack table is landing at the bottom of Team B's playfield (flashing in Alice's cyan). The greyed-out rows at the bottom of each playfield are garbage lines from earlier exchanges, each framed in the color of the player whose line clear sent it. Every garbage line comes with one hole, and the lines of one attack line up their holes into a well — fill it and they clear like any other line.*

**An example of a peer-to-peer distributed blackboard system built using 'nothing but [NATS](https://nats.io)' for humans or agents to cooperate or compete towards a common goal and disguised as a fun real-time, multiplayer, modern-rules cooperative/competitive block-stacking game**
...
Option 1: Nothing to install — open **https://jetris.net/** and hit **Play**: the browser version (WebAssembly) runs right on that page, always the latest release. It plays on a tablet too: on a touch screen the game lays out a thumb-sized D-pad and buttons beside the playfield, so you play by tapping — or by gestures on the playfield, as in the phone versions: swipe left or right to shift the piece, tap the left or right half of the board to rotate it, tap with two fingers to turn it half way round, drag down to soft-drop (the piece still steers), flick down to hard-drop, swipe up to hold. A game controller works too, on the page and in the desktop builds alike — an Xbox, PlayStation or Switch pad over Bluetooth or USB: the D-pad and left stick move and soft-drop, ○/B and ✕/A turn the piece, □/X turns it half way round, D-pad up (or △/Y) hard-drops and the shoulders hold; on the page the browser only shows the pad to the game after you press a button on it. If the WiFi drops mid-game the HUD shows **LINK LOST** with a running clock while the connection is re-established, and the moves you made meanwhile land once it is back — none are lost. That page is served over https, so the NATS server you pick on its login screen has to be reachable over `wss://` — the public `demo.nats.io` server is preselected, and favorites this page can't dial (`nats://`, and plain `ws://`) are greyed out.
...
A finished game has a link of its own as well. The replay screen's **Share** button puts that replay's link on the screen as a QR code, with the link under it and a **Copy link** button: it is the same join page with the game named — `join.html?server=…&name=…&replay=<gameID>` — so whoever opens it (or scans it) lands straight on the replay screen with that game playing — no name asked, the game deals one (`Watcher_GhostPiece07`). The desktop build sends the link to the GitHub Pages copy of the game and the server's WebSocket address (the official servers are bookmarked both ways, so a desktop on the `nats://` one still knows it; a LAN party shares its own page); the desktop opens the same game with `jetris --server … --replay <gameID>` (add `--name` to watch under your own name). Both buttons are also on the game-over box the moment a game ends, beside **Back to Lobby**, so the game you just played is pinned or shared without a trip through the history. Beside Share sits **Pin**: a pinned replay is never cleaned up — replays are otherwise kept only while the game is among the 25 most recent or the top 10 of its bucket — until someone presses **Unpin**; the history marks pinned games `PINNED`, and its **Pinned only** box lists nothing else.

An **open game** has a link too, for the people you want in it. Every open game's row in the lobby carries a **Share** button while the game takes joiners — before it starts and, since an open game's seats are anyone's at any time, while it runs — which puts the game's link on the screen as a QR code, with the link under it and **Copy link**: the join page with the game named, `join.html?server=…&name=…&game=<gameID>`. Whoever opens it (or scans it) is told they are about to join game `3f2a9b1c` on that server and asked for a name, which they can leave blank to play anonymously (`Anonymous_TSpinGod07`), and then lands straight in the game, in a free seat — on the emptiest team of a teams game. A seat that is gone by then (the game over, or full) is reported under the lobby banner, the row still there to spectate from. The desktop build opens the same game with `jetris --server … --join <gameID>`: with `--name` it goes straight in, without it the login screen says which game Play joins.

`-page` is where the browser build is served from: the GitHub Pages copy by default (`https://jetris.net/join.html?server=wss://host:443&name=My%20server`), or your own `python3 -m http.server -d dist/web 8080` — the phones have to reach that page as well as the NATS server, so on a LAN give both real addresses rather than `localhost`. The URL must be the server's **WebSocket** address (`ws://` or `wss://`, its `websocket {}` listener), since that is all a browser can dial — and a page served over https can only reach a `wss://` one. No credentials go in a join link: anyone who can see the QR code can read what is in it, so a server that needs a user and password is joined through the login screen. The desktop build takes the same shortcut from the command line: `jetris --server nats://192.168.1.20:4222 --name alice` goes straight to that lobby.
...
- Cooperative: 1 or more players share one wide board (10 columns plus the game's extra columns per seat beyond the first), each controlling their own piece. A crew of one is a **solo game** on the standard board, played for the highest score — the record it competes for is the best solo co-op score. Pieces can't overlap; the shared board uses per-cell CAS with merge-retry so neither player ever clobbers the other's in-flight piece — while a solo game, having no other writer, guards nothing: it is played on the local board at full speed and journaled to the stream without expectations (§5.11). Score is shared — and every mode scores by the modern scoring table: singles to quads at the level's multiplier, T-spins, Back-to-Back, combos, perfect clears and drop points (see [`jetris-gameplays.md`](jetris-gameplays.md) § Scoring).
...
Prebuilt binaries for Linux, macOS, and Windows (amd64 + arm64) are produced on tagged releases by `.github/workflows/release.yml`, which also builds the browser version, attaches it to the release as `jetris-<tag>-web.tar.gz` (for self-hosting) and deploys it to GitHub Pages at https://jetris.net/.

### Browser build (WebAssembly)

The same client also builds for the browser — Gio renders to a WebGL canvas and the game runs unchanged, as a wasm module. The latest release is hosted on GitHub Pages at **https://jetris.net/**: a landing page with a **Play** button (`https://jetris.net/#play` skips straight to the game), redeployed by the release workflow on every tag. To build and serve it yourself:
...
Open `http://localhost:8080/` (add `?server=wss://host:port` to preselect a server, the browser build's stand-in for `--server`; `user`/`password` work too, `name=` labels the server, `player=` gives the player's name — with that one the game skips its login screen and joins the lobby outright — `replay=<gameID>` opens that game's replay on landing, which is what a replay's share link carries — with no `player=` under a dealt `Watcher_` name, login screen skipped — and `game=<gameID>` takes a seat in that open game on landing, which is what a game's share link carries — with no `player=` the login screen is shown, saying which game Play joins). Differences from the desktop build, all behind `//go:build js` files:

- **The join page** (`web/join.html`, `dist/web/join.html`). A link that carries a server — `join.html?server=wss://host:443&name=My%20server` — opens a page whose only question is the player's name (left blank, JOIN deals one from the same list of handles as the login screen), and hands all three parameters to the game page, which connects and goes straight to that server's lobby. It is what the LAN party mode's QR code, the QR codes of `scripts/gen-qr.go`, the replay screen's Share and the lobby's Share point at (a replay's share link adds `&replay=<gameID>`, and the page then asks nothing: it hands the server and game ID straight on, and the game watches under a dealt `Watcher_` name; an open game's share link adds `&game=<gameID>`, and the page says which game it is about to join — the first eight characters of the ID, as the lobby row shows it — over a name field that may be left blank to play anonymously), so hosting a game is putting one code on a screen (see *Hosting a game* at the top of this README); a LAN party's server sends its bare `http://<ip>:8080/` there as well, so the address typed into a phone's browser asks the same one question. It refuses, before the ~25 MB download rather than after, what the game would only find out at the end of it: a `nats://` server, a `ws://` one on an https page (mixed content), and a name the game would reject (`config.ValidatePlayerName`). Whatever still fails — the server is down, the name is taken — falls back to the ordinary login screen with the error shown, the name filled in and every other server one tap away. It loads no wasm of its own: the game page carries the touch workarounds a tablet needs, and there is no sense in two copies of them. The auto-join itself is `config.Config.PlayerName` (`--name` on the desktop): the login screen plays its own Play button on its first frame, once.
...
To play multiplayer, **run more instances pointed at the same NATS server** — each instance is one peer. Create a game in the lobby, have the others join it, ready up, and play. The create wizard walks through three steps. **Game type** first: a **single playfield** everyone shares — the number of players first, and with two or more the choice between **co-op**, all players scoring together, and **competitive**, each player scored on their own with the top score winning (the lobby row calls it `competitive · one board`); one player alone is a solo game; or **multiple playfields** (2–6), a team on each, every team scoring together and every line a team clears sending garbage to the others, with the number of players per playfield — one per playfield is a board each, the classic last-player-standing game, two or more a teams game (`2v2`). Then the **rules step**: the **game length** — **until top out**, or **a number of lines** (40 unless you say otherwise): the first playfield to clear that many lines wins, or, on a single playfield, the game ends there — the crew is done, a competitive board crowns its top scorer — and the HUD counts `LINES 12 / 40`; the row tags such a game `40 lines` — or, on a single playfield, **survival**: the floor rises until you top out. Garbage rows rise from the bottom of the board on a clock of their own that quickens with the level, one of six tiers sets the pace (**Easy** one row every 2.5 s at level 1 down to every second by level 15, **Normal** — the default — one to four rows every 3 s down to 1.5 s, **Hard** four rows every 5 s down to every second, and below Easy, one row at a time and each rising half as often as the tier above it, **Very easy** every 5 s down to 2 s, **Super easy** every 10 s down to 4 s and **Too easy** every 20 s down to 8 s), every raised row comes with at least one hole so it can be cleared — the holes line up into a well that moves every four rows — and the game ends at the first top-out with the **time survived** as its result: the HUD ticks `SURVIVED 1:23`, the game-over box says `SURVIVED 3:42 · NORMAL`, the history ranks survival games by their time (longest first, in a bucket of their own per tier), the fireworks play for a new best time at that tier and seat count, and the row tags such a game `survival (normal)`. Below it a single radio: **Modern** (the default) plays every rule at the setting closest to modern play — 6 next pieces, the ghost piece, the hold queue, the board 4 columns wider per player, and, with several playfields, modern garbage (1 hole per row, the rows of one attack lined up into a well, attacks by the modern attack table) — and the lobby row tags such a game `modern`; **Custom** exposes each rule, the board's growth first: **"Extra columns per player" (4–10, default 4)** and **"Extra rows per player" (0–10, default 0)** — a shared board is 10 columns and 20 rows for its first player and that much more for every player after (the row shows `board 14×20`), the hidden rows above never move — and **"Distribute the pieces between the players of a playfield"** (off by default; see below). The **"Next" count (0-6, default 6)** sets how many upcoming pieces the game reveals: players see them in the HUD's NEXT panel, the lobby row shows `next 2`-style tags, and agents get exactly the same lookahead (it is part of the game's meta record, so the whole game — humans and agents — plays with one preview horizon; 0 means nobody sees anything coming). The **"Show ghost piece" checkbox (on by default)** decides whether the game draws each player's hard-drop landing preview — also a per-game rule in the meta, one setting for everyone. The **"Hold piece" checkbox (off by default)** switches on the hold queue: press **C** — or the HOLD button of the on-screen pad, or tap the HOLD box beside the playfield, or swipe up on the playfield — to set the falling piece aside and play the next one, or to swap the held piece back in later, once per piece; the row tags such a game `hold`. With several playfields that step also sets **"Garbage holes" (0-4, default 1)**: how many empty cells every garbage row an attack sends comes with. At 0 garbage rows are solid and can never be cleared (the classic Jetris rule); with holes, a garbage row clears like any other line once its holes are filled — scoring and counter-attacking as usual — and the rows of one attack line up their holes into a well, unless **"Random hole positions"** (off by default) is checked, in which case every garbage row draws its own holes. A third checkbox, **"Modern attack table"** (off by default), switches the attack strength from one row per cleared line to the modern attack table — a single sends nothing, a double 1 row, a triple 2, a quad 4, a T-Spin single/double/triple 2/4/6, a Back-to-Back clear one to three rows more, a perfect clear 10 more. The lobby row tags such games `holes 2` / `random holes 2` / `modern attacks`-style. **Distributing the pieces** — off by default; offered wherever a playfield has more than one player — deals the seven piece types out between the players of a playfield instead of everyone playing the same 7-bag: every type goes to somebody, nobody gets fewer than one, and each player only ever plays their own; the playfield still has the whole bag, but only between its players, so the one holding the I is the only one who can hand the board an I. Every playfield is dealt the same hands (the deal comes from the game's seed), each player's ration is listed under their name in the HUD legend, and the lobby row tags the game `split pieces`; in an open game the deal follows the players present, re-dealt whenever someone comes or goes. The wizard's last step, **players**, first lets a teams game name its teams — each playfield's team is called after a piece colour (Green, Blue, Amber, Violet, Teal, Coral) unless you type another name — then makes the game **invite-only** — select who to invite (including specific agents, and per-team in teams mode); each selection sends its invitation on the spot, and deselecting retracts it — or **open**: anyone in the lobby can take a free seat, and **players can enter and leave the game at any time**, while it runs too. An open game starts as soon as every playfield has a player who clicked ready (the countdown follows), whatever seats are still free; a player who joins later takes a free seat on the live board and plays right away, no ready step; and "Allow agents to join" with a max-agents count (per team in a teams game), on that same step, lets idle agents take seats, with "Agents pause when alone" beneath it for an agent left as the game's only player to wait for someone to join rather than play on by itself. Leaving an open game with "Back to Lobby" takes your falling piece off the board and frees your seat for anyone (the lobby lists the game to join again); a player who crashes or drops off leaves a piece behind, which the other players' engines take off the board after ten seconds without a move. You start selected yourself — creating an invitation game counts as accepting your own invitation, so a seat is taken immediately; deselect yourself to host a game you'll spectate instead. Invited players get a pop-up (showing who has already joined) to accept or decline — agents accept automatically — and the creator watches each invitation live: waiting, joined, ready, or declined. Once every seat is filled the creator is carried straight to the game (playing, or spectating if they opted out). Leaving an invite-only game with "Back to Lobby" keeps your seat: the lobby lists it as **joined** (or **playing** once started, after an are-you-sure prompt) with a **Rejoin** button.

### Game controllers

A gamepad drives the piece on every build; the lobby, the menus and the chat stay on the mouse, the keyboard and the touch screen. The scheme is fixed and the legend's **GAMEPAD** section (the game's menu column, the lobby's) shows it from the first press of a pad on: **D-pad ← → and the left stick** move (held, they slide on the same DAS/ARR tuning as the keys), **D-pad ↓ / stick ↓** soft-drops, **○ / B** rotates clockwise, **✕ / A** counter-clockwise, **□ / X** half way round, **D-pad ↑ or △ / Y** hard-drops (one drop per press, through the accidental-drop guard like the space bar), and **LB, RB, LT, RT** hold. The pad keeps driving the piece while you are typing in the chat — it cannot type, so there is nothing to hand over.

- **macOS:** Apple's GameController framework — Xbox, PlayStation (DualShock 4, DualSense), Switch Pro and MFi pads, paired in System Settings or plugged in over USB. Plug in before or after launch; the first pad attached drives.
- **Windows:** XInput — any Xbox controller, and whatever presents itself as one (Steam Input, DS4Windows). A PlayStation pad on its own is not seen.
- **Linux:** the kernel's joystick device, `/dev/input/js0` to `js3`, Xbox (`xpad`) and PlayStation (`hid-playstation`) alike. The udev rules on most distributions give the seated user the joystick nodes; if the terminal logs `gamepad: /dev/input/js0: permission denied`, add yourself to the `input` group or a udev rule.
- **Browser:** the Gamepad API (Chrome, Firefox, Safari), over https or `localhost`. Every browser hides a pad from the page until a button is pressed on it while the page has the focus — so press one, then play.
...
- **The mic button** sits beside the menu button in the game's top bar — and in the lobby's, where the room is everyone in the lobby on that server (`jetris.voice.lobby.all.<player>`). It shows a microphone with a red slash through it while you are muted: tap it to open the microphone (the OS or the browser asks for permission the first time), tap again to mute. It lights green while your voice is going out. **The switch follows you from room to room**: open in the lobby, it is open in the game you join or spectate next, and in the lobby you come back to; muted in the game, muted in the lobby after. **A fresh connection to a lobby always starts muted.** A replay or the archive leaves the lobby's room; coming back rejoins it, the microphone as you left it. The lobby's room and a game's are separate rooms: nothing said in one is heard in the other.
...
internal/
  ...
  game/                pure modern rules: pieces, rotation (SRS, SRS-X half turns), collision, line clears
...
* Jetris is a demonstration that the blackboard pattern — shared state, concurrency control, and real-time push, over one substrate — is a first-class thing you can build directly on NATS.io. The players happen to be human as well as agents and the task happens to be multi-player modern-rules block-stacking *
