...
// — a quad, or any T-spin that cleared lines — the clears that start and
...
// four-line clear is a quad, whatever turned the piece), and a three-line
...
func (c Clear) basePoints() int {
	...
	return [5]int{0, 100, 300, 500, 800}[n] // nothing, Single, Double, Triple, quad
}
...
// sends nothing, a double 1, a triple 2, a quad 4; a Mini T-Spin single
...
// Name is the clear's name — "QUAD", "T-SPIN DOUBLE",
...
func (c Clear) Name() string {
	...
	name := [5]string{"", "SINGLE", "DOUBLE", "TRIPLE", "QUAD"}[n]
	if spin == "" {
		return name
	}
	return spin + " " + name
