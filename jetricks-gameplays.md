...
- A "Run own NATS server" option is **always** available: hitting Play with it selected starts a JetStream-enabled NATS server **inside the Jetricks process itself** — listening on port 4222 on all interfaces, storing its stream data in the local `nats-data` directory — and connects to it over loopback. The lobby then shows the server's LAN address (`YOUR SERVER nats://<ip>:4222 — share this address so others can join you`) so the host can invite other players, who connect to that address via the NATS URL option. The embedded server keeps running until the window closes, so quitting to the login screen doesn't kick connected friends.
