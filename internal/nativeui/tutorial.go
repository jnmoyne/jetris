	...
	steps := []tutStep{
		...
		{scene: tutSceneWizard, title: "GAME RULES", targets: []string{tutWizard}, prep: wizard(wizStepNext),
			body: "Guideline sets every rule to its Tetris Guideline setting: six pieces shown in the NEXT well, the ghost that marks where a hard drop lands, and the hold queue. " +
				"Custom lets you set each one yourself — the bag the pieces are dealt from too: the 7-bag, a double bag of fourteen, or no bag at all — whether the hidden rows above the playfield show through smoked glass, and, in the modes that raise garbage, how many holes a garbage row has, whether the rows of one attack line them up, and whether attacks follow the Guideline table: a single sends nothing, a double one row, a triple two, a Tetris four."},
