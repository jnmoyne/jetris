...
- Cooperative: 1 or more players share one wide board (10 columns plus the game's extra columns per seat beyond the first), each controlling their own piece. A crew of one is a **solo game** on the standard board, played for the highest score — the record it competes for is the best solo co-op score. Pieces can't overlap; the shared board uses per-cell CAS with merge-retry so neither player ever clobbers the other's in-flight piece. Score is shared — and every mode scores by the Tetris Guideline table: singles to Tetrises at the level's multiplier, T-spins, Back-to-Back, combos, perfect clears and drop points (see [`jetris-gameplays.md`](jetris-gameplays.md) § Scoring).
