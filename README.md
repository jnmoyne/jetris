...
- Cooperative: 2+ players share one wide board (`playerCount × 10` columns), each controlling their own piece. Pieces can't overlap; the shared board uses per-cell CAS with merge-retry so neither player ever clobbers the other's in-flight piece. Score is shared — and every mode scores by the Tetris Guideline table: singles to Tetrises at the level's multiplier, T-spins, Back-to-Back, combos, perfect clears and drop points (see [`jetris-gameplays.md`](jetris-gameplays.md) § Scoring).
