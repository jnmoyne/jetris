...
func TestTeamsJoinAssignsSlotsAndRejectsFullTeam(t *testing.T) {
	...
	gameID, err := lbs[0].CreateGame(ctx, config.ModeTeams, 4, 2, 0, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTeamsConcurrentJoinsRespectCapacity(t *testing.T) {
	...
	gameID, err := lbs[0].CreateGame(ctx, config.ModeTeams, 2, 1, 0, config.GameRules{Ghost: true}, false)
	if err != nil {
		t.Fatal(err)
	}
