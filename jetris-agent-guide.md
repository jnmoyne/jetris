...
| `…player.<player>.playfield.garbage` / `…team.<t>.playfield.garbage` / `jetris.game.<id>.playfield.garbage` | `{"total": N, "by": idx, "raises": K}` | the board's GARBAGE register: cumulative garbage rows OWED, CAS-added by attackers — the crew's board has one in a SURVIVAL game only, where the floor's clock advances it with a write-once CAS and `raises` counts the raises (§4.4) |
| `…player.<player>.playfield.txn` / `…team.<t>.playfield.txn` / `jetris.game.<id>.playfield.txn` | `{"applied": N, "op": …, "topped": […], "full": bool, "by": idx}` | the board's TXN register: rows APPLIED + the exactly-once gate every bulk transform CASes through (§4.4); the crew's board has one in a survival game only |
...
- **Attacking (you cleared N lines).** The rows you owe are N — or, when the
  meta's `guideline_garbage` is true, the modern attack table: 0 for a single, 1
...
- **The rising floor (a survival game: the meta's `survival` is
  `"too_easy"`, `"super_easy"`, `"very_easy"`, `"easy"`, `"normal"` or
  `"hard"`; cooperative only).** Nobody attacks anybody: the
  crew's own board carries the garbage and txn registers
  (`jetris.game.<id>.playfield.garbage` / `.txn`, on the same `playfield.>`
  filter as its cells), and the garbage is the FLOOR's. Run a clock from the
  moment the game is `in_progress`: a raise is due `interval(level)` after
  the last one landed, where the level is the board's (`1 + lines / 10`,
  15 at most — a raised row clears like any line and counts) and the
  interval is a straight line from the tier's level-1 start to its level-15
  end, in whole nanoseconds — too_easy 20 s → 8 s, super_easy 10 s → 4 s,
  very_easy 5 s → 2 s, easy 2.5 s → 1.0 s, normal 3.0 s → 1.5 s, hard
  5.0 s → 1.0 s: `start − (start − end) × (level − 1) / 14`. When it
  fires, publish ONE message to the garbage register — `{"total": T +
  rows, "raises": K + 1, "by": <you>}` — with `Nats-Expected-Last-Subject-
  Sequence` at the register's last sequence as YOU last saw it (0 if never
  written), where `rows` is what the (K+1)-th raise brings: 1 on easy and
  the three tiers below it, 4 on hard, and on normal `1 + PCG(seed ^
  0x5375727620726f77, K+1).IntN(4)` (Go's
  `math/rand/v2` PCG, the game's `seed`). Every engine on the board keeps
  this clock, and the CAS lets exactly ONE raise land per tick: a rejection
  means a crewmate's landed first — do NOT re-add (the contrast with an
  attack's CAS-add). Re-arm your clock from every message on the register
  — yours or theirs — and treat a tick that finds the register moved since
  you armed it as a no-op. Then apply the deficit exactly as an attack's
  (the gated `shrink`, any alive seat may; the gate admits one), with the
  holes drawn by the FLOOR's rule: every raised row has an ordinal — the
  board's `applied` count when the raise lands, plus its index in the raise
  — and the rows sharing `ordinal / 4` share one draw, so the holes line up
  into a well that moves every four rows: `sort(PCG(seed ^ 0x53757276206f6c65,
  ordinal / 4).Perm(width)[:holes])`, `holes` the meta's `garbage_holes`
  floored at 1 and capped at width−1; with `random_garbage_holes`, the
  stream is the `ordinal` itself. A raise that lists you in `topped`, or
  marks the board `full`, is your top-out — and the crew's game over, as any
  co-op top-out is. Because the floor's shrink is NoCAS, your line clears
  and vacates on a survival board must take the gate like a team's (`"op":
  "clear"` / `"vacate"`), never the merge-retry batch. The archive record
  carries `survival` with the tier; such a game is ranked by the time it
  survived. The reference agent's `survival.go` is the port, pinned against
  the GUI's numbers by its parity tests.
...
### 4.6 Scoring: your locks, by the modern scoring table

Every mode scores by the modern scoring table (gameplays §2 Scoring; the
...
- **The clear** — Single 100, Double 300, Triple 500, Quad 800; a T-spin
  ...
  1-4 lines (3200 for a Back-to-Back quad) — nothing locked left on the
...
- [ ] On a shared playfield, your pieces spawned at your point among the seats PRESENT — one `extra_columns` step from your neighbours, the group centred on the board, alone in the middle of it — recomputed from the listing's roster at every spawn (§2/§3/§5 of the gameplays)
...
- [ ] `survival` honored: the crew's garbage register raised on the tier's clock with a write-once CAS (`raises` advanced, a rejection never re-added, the clock re-armed from every register message), the owed rows applied on the crew's board through the gate with the moving-well holes off the seed and the row ordinals, your clears and vacates on that board gated, and the archive record carrying the tier (§4.4)
- [ ] Attacks delivered by CAS-adding victims' garbage registers (never events), sized one row per line — or by the modern attack table (0/1/2/4 for plain clears, the T-spin rows, the Back-to-Back and perfect-clear bonuses) when the meta's `guideline_garbage` is true (§4.4)
- [ ] Your own locks scored by the modern scoring table and announced with your cumulative totals — every clear, and on a shared board every lock that scored (§4.6)
